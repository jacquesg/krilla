...
//! [`ColorConversion`] dispatch wired into
//! `ContentBuilder`.
//!
...
use krilla::color::{cmyk, luma, rgb, ColorConversion};
...
/// projection policy.
fn settings_with_policy(policy: ColorConversion) -> SerializeSettings {
    SerializeSettings {
        color_conversion: policy,
        ..settings_1()
    }
}
...
/// content stream).
fn render_filled_rect(policy: ColorConversion, paint: krilla::paint::Paint) -> Vec<u8> {
...
// --- 3x3 matrix: policy in {ForceRgb, ForceCmyk, ForceGray}
//                source in {RGB red, CMYK red, Luma 50%} ---------------

#[test]
fn force_rgb_from_rgb_emits_rg() {
    let pdf = render_filled_rect(ColorConversion::ForceRgb, rgb::Color::new(255, 0, 0).into());
...
fn force_rgb_from_luma_emits_rg() {
    ...
    let pdf = render_filled_rect(ColorConversion::ForceRgb, luma::Color::new(128).into());
    let line = fill_operator_line(&pdf, "rg");
...
fn force_cmyk_from_luma_emits_k_pure_black() {
    ...
    let pdf = render_filled_rect(ColorConversion::ForceCmyk, luma::Color::new(128).into());
    let line = fill_operator_line(&pdf, "k");
    let parts: Vec<&str> = line.split_whitespace().collect();
...
fn force_grey_from_luma_emits_g() {
    let pdf = render_filled_rect(ColorConversion::ForceGray, luma::Color::new(128).into());
    fill_operator_line(&pdf, "g");
}
...
fn auto_policy_is_pass_through_rgb() {
    ...
    let pdf = render_filled_rect(ColorConversion::Auto, rgb::Color::new(255, 0, 0).into());
