...
#[test]
fn encryption_encrypts_chunk_content() {
    // Content objects (annotations, page streams, fonts, images) are built as
    // separate chunks and merged at finish, so encryption must be applied at
    // chunk-build time. Verify a distinctive string routed through a
    // link-annotation chunk is NOT emitted in clear under encryption (ISO
    // 32000-2 §7.6.2: "Encryption applies to all strings and streams in the
    // document's PDF file").
    const MARKER: &str = "https://SECRETMARKERabc123.example/";
    let build = |settings: SerializeSettings| -> Vec<u8> {
        let mut doc = Document::new_with(settings);
        let mut page = doc.start_page_with(PageSettings::from_wh(72.0, 72.0).unwrap());
        page.add_annotation(Annotation::from(LinkAnnotation::new(
            Rect::from_xywh(0.0, 0.0, 72.0, 72.0).unwrap(),
            Target::Action(LinkAction::new(MARKER.to_string()).into()),
        )));
        page.finish();
        doc.finish().unwrap()
    };
    // Sanity: without encryption the URI is present in clear, so a false pass
    // is impossible.
    assert!(
        contains(&build(crate::settings_1()), MARKER.as_bytes()),
        "unencrypted output should contain the link URI in clear",
    );
    // With encryption the annotation chunk is encrypted, so the URI is gone.
    assert!(
        !contains(&build(encrypted_settings()), MARKER.as_bytes()),
        "link-annotation URI leaked in clear from an encrypted document \
         — content chunks are not being encrypted",
    );
}

#[test]
fn encrypt_metadata_false_keeps_metadata_stream_plaintext() {
    // With encryption active but `encrypt_metadata` disabled, the document
    // XMP /Metadata stream must be written in clear and carry an Identity
    // /Crypt filter (ISO 32000-2 §7.6.6): its title is readable while every
    // other string and stream stays AES-encrypted.
    let settings = SerializeSettings {
        encryption: Some(Encryption::new("alice", "bob").with_encrypt_metadata(false)),
        xmp_metadata: true,
        ..crate::settings_1()
    };
    let pdf = build_doc(settings, "Public XMP Title");
    assert!(
        contains(&pdf, b"/EncryptMetadata false"),
        "/EncryptMetadata false must be advertised",
    );
    assert!(
        contains(&pdf, b"Public XMP Title"),
        "XMP title must be plaintext when encrypt_metadata is false",
    );
    assert!(
        contains(&pdf, b"CryptFilterDecodeParms"),
        "the /Metadata stream must carry an Identity /Crypt filter",
    );
}
