...
fn overprint_extgstate_entries_emitted() {
    ...
    surface.push_overprint(Overprint::preserve());
    surface.set_fill(Some(green_fill(1.0)));
    surface.draw_path(&path);
    surface.pop();
    surface.finish();
...
/// ISO 32000-2 Table 57 coupling-avoidance: when the caller sets ONLY
/// the stroking overprint flag (`/OP`), krilla must still emit the
/// non-stroking flag (`/op`) explicitly at its PDF default (`false`),
/// rather than omitting it. A bare `/OP` would otherwise couple the
/// non-stroking overprint parameter onto `/OP` in a conforming viewer,
/// so both keys must appear in the `ExtGState`.
#[test]
fn overprint_stroking_only_emits_both_flags() {
    use krilla::geom::PathBuilder;
    use krilla::overprint::Overprint;
    use krilla::page::PageSettings;
    use krilla::Document;
    ...
    pb.close();
    let path = pb.finish().unwrap();
    // Only the stroking flag is set; the non-stroking flag is left unset.
    surface.push_overprint(Overprint {
        stroking: Some(true),
        ..Default::default()
    });
    surface.set_fill(Some(green_fill(1.0)));
    ...
    // The stroking flag the caller set.
    assert!(
        pool.contains("/OP true"),
        "stroking overprint /OP true should be emitted; pool excerpt: {}",
        &pool[..pool.len().min(2000)],
    );
    // The non-stroking flag, emitted explicitly at its PDF default so a
    // viewer cannot couple /op onto /OP (ISO 32000-2 Table 57).
    assert!(
        pool.contains("/op false"),
        "non-stroking overprint /op false should be emitted explicitly to avoid coupling",
    );
}
