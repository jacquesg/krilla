...
#[test]
fn validate_pdf_x4p_annotation_colors_are_not_validated() {
    // settings_37 is PDF/X-4p with an RGB output intent. An annotation is a
    // non-print element (ISO 15930-7 §3.15), so its /C border colour is exempt
    // from the output-intent colour check (§6.1.3) whatever colour space it
    // uses. The annotation sits outside the print area; the page fill is RGB
    // (ICC-wrapped to match the RGB intent) so only the annotation border
    // colour varies between runs.
    ...
    // Both borders are permitted: an annotation is a non-print element
    // (ISO 15930-7 §3.15) and non-print elements "may make use of any PDF
    // colour space" (§6.1.3), so neither the RGB nor the CMYK annotation
    // border is validated against the RGB output intent.
    assert!(
        run(rgb::Color::new(255, 0, 0).into()).is_ok(),
        "an RGB annotation border is a non-print colour and must not be validated",
    );
    assert!(
        run(cmyk::Color::new(0, 0, 0, 255).into()).is_ok(),
        "a CMYK annotation border is a non-print colour and must not be validated",
    );
}
...
fn validate_pdf_x_prohibits_verdicts_by_level() {
    ...
    let cases: Vec<(ValidationError, [bool; 6])> = vec![
        ...
        (
            // .notdef is forbidden only by X-4/-4p (ISO 15930-7 §6.5.2) and
            // X-6/-6p (ISO 15930-9 §6.8.3); X-1a/X-3 impose no such rule.
