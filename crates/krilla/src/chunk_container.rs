...
impl ChunkContainer {
    ...
    pub(crate) fn finish(
        ...
        let document_js_refs: Vec<(String, Ref)> = self
            .metadata
            .as_ref()
            .map(|m| {
                m.document_javascripts
                    .iter()
                    .map(|(name, source)| {
        ...
        let document_event_refs: Vec<(crate::interchange::metadata::DocumentEvent, Ref)> = self
            .metadata
            .as_ref()
            .map(|m| {
                m.document_event_scripts
                    .iter()
                    .map(|(event, source)| {
                        let ref_ = remapped_ref.bump();
                        let mut action = pdf.indirect(ref_).start::<pdf_writer::writers::Action>();
            ...
            || self.non_stream.struct_tree_root.is_some()
        {
            ...
            let vp_struct = self
                .metadata
                .as_ref()
                .map(|m| m.viewer_preferences.clone())
                .unwrap_or_default();
            ...
            if let Some(mode) = self.metadata.as_ref().and_then(|m| m.page_mode) {
