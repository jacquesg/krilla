...
pub enum RegularColor {
    ...
    /// calibrated colour space (ISO 32000-2 §8.6.5.3).
    ///
...
/// CalRGB parameter dictionary (ISO 32000-2 §8.6.5.3 Table 63).
///
...
// Compare every float field by bit pattern so `PartialEq`/`Eq` and
// `Hash` agree. A derived (IEEE) `PartialEq` would make `+0.0 == -0.0`
// while the manual `Hash` below hashes their distinct `to_bits`, and
// would leave `NaN != NaN` — breaking `Eq` reflexivity.
impl PartialEq for CalRgbParams {
    fn eq(&self, other: &Self) -> bool {
        self.white_point.map(f32::to_bits) == other.white_point.map(f32::to_bits)
            && self.black_point.map(|v| v.map(f32::to_bits))
                == other.black_point.map(|v| v.map(f32::to_bits))
            && self.gamma.map(|v| v.map(f32::to_bits)) == other.gamma.map(|v| v.map(f32::to_bits))
            && self.matrix.map(|v| v.map(f32::to_bits)) == other.matrix.map(|v| v.map(f32::to_bits))
    }
}
...
/// CalGray parameter dictionary (ISO 32000-2 §8.6.5.2 Table 62).
#[derive(Debug, Clone, Copy)]
...
// Bitwise float comparison so `PartialEq`/`Eq` and `Hash` agree; see
// [`CalRgbParams`] for the ±0.0 / NaN rationale.
impl PartialEq for CalGrayParams {
    fn eq(&self, other: &Self) -> bool {
        self.white_point.map(f32::to_bits) == other.white_point.map(f32::to_bits)
            && self.black_point.map(|v| v.map(f32::to_bits))
                == other.black_point.map(|v| v.map(f32::to_bits))
            && self.gamma.map(f32::to_bits) == other.gamma.map(f32::to_bits)
    }
}
...
/// Lab parameter dictionary (ISO 32000-2 §8.6.5.4 Table 64).
///
...
// Bitwise float comparison so `PartialEq`/`Eq` and `Hash` agree; see
// [`CalRgbParams`] for the ±0.0 / NaN rationale.
impl PartialEq for LabParams {
    fn eq(&self, other: &Self) -> bool {
        self.white_point.map(f32::to_bits) == other.white_point.map(f32::to_bits)
            && self.black_point.map(|v| v.map(f32::to_bits))
                == other.black_point.map(|v| v.map(f32::to_bits))
            && self.range.map(|v| v.map(f32::to_bits)) == other.range.map(|v| v.map(f32::to_bits))
    }
}
...
pub(crate) fn separation_to_regular(spot: &separation::Color) -> RegularColor {
    ...
    match &spot.space.fallback {
        ...
        .into(),
        RegularColor::Luma(c) => {
            luma::Color::new(unit_to_u8(1.0 - (1.0 - u8_to_unit(c.0)) * tint)).into()
        }
        ...
        // White-anchor them to match the additive (C0 = 1.0) tint
        // transform the writer emits for every non-CMYK fallback — a
        // defensive device-space approximation, since the constructors
        // used in practice route through the process arms above.
        ...
        RegularColor::CalGray { component, .. } => {
            luma::Color::new(unit_to_u8(1.0 - (1.0 - *component) * tint)).into()
        }
        RegularColor::Lab { components, .. } => {
            // Anchor on Lab white (L = 100, a = b = 0) rather than
            // device black: interpolate L down from 100 and a/b out
            // from 0 by the tint, then rescale into [0, 1].
            let l = 100.0 - (100.0 - components[0]) * tint;
            let a = components[1] * tint;
            let b = components[2] * tint;
            rgb::Color::new(
                unit_to_u8((l / 100.0).clamp(0.0, 1.0)),
                unit_to_u8(((a + 100.0) / 200.0).clamp(0.0, 1.0)),
                unit_to_u8(((b + 100.0) / 200.0).clamp(0.0, 1.0)),
            )
            .into()
        }
    }
...
pub mod devicen {
    ...
    ///
    /// Every PDF/A and PDF/X profile permits DeviceN; the only
    /// conformance constraint is on the alternate colour space, which is
    /// validated when that space is registered.
    #[derive(Debug, Eq, PartialEq, Hash, Clone)]
...
pub(crate) enum CieBasedColorSpace {
    ...
    /// CalRGB PDF-native calibrated RGB space (ISO 32000-2 §8.6.5.3).
    /// Emits an inline `[/CalRGB <<WhitePoint … >>]` colour-space
    /// array as a `/CS<n>` resource.
    CalRgb(CalRgbColorSpace),
    /// CalGray PDF-native calibrated grey space (ISO 32000-2 §8.6.5.2).
...
/// inline `[/CalGray <<…>>]` array (ISO 32000-2 §8.6.5.2).
#[derive(Debug, Eq, PartialEq, Hash, Clone, Copy)]
...
mod tests {
    ...
    fn cal_rgb_colour_space_array_lands_in_pdf() {
        let params = CalRgbParams {
            // D65 white point per ISO 32000-2 §8.6.5.3.
    ...
    fn cal_gray_colour_space_array_lands_in_pdf() {
        ...
        // ISO 32000-2 Table 62 types CalGray `Gamma` as a scalar
        // `number`, not an array (cf. CalRGB, whose Gamma is a
        // three-element array). Pin the scalar form so a regression to
        // `/Gamma [ … ]` is caught.
        assert!(
            contains(&pdf, b"/Gamma 2.2"),
            "CalGray /Gamma should be emitted as a scalar number (Table 62)"
        );
        assert!(
            !contains(&pdf, b"/Gamma ["),
            "CalGray /Gamma must not be emitted as an array"
        );
        // BlackPoint is optional; when None it is omitted entirely.
        assert!(
            !contains(&pdf, b"/BlackPoint"),
            "CalGray /BlackPoint should be absent when None"
        );
    }
