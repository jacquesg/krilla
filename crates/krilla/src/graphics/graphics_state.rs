...
impl Cacheable for ExtGState {
    fn serialize(
        ...
        // ISO 32000-2 Table 57: an `/OP` value written without an
        // accompanying `/op` couples BOTH the stroking and non-stroking
        // overprint parameters to `/OP`. krilla's graphics state
        // accumulates only the flags the caller set (any inherited value
        // is materialised into `Some` by `combine`), so an unset flag
        // genuinely denotes the default (`false`). Emit both explicitly
        // whenever either is set, keeping the two parameters independent
        // as the `Overprint` API promises.
        if self.0.stroking_overprint.is_some() || self.0.non_stroking_overprint.is_some() {
            ext_st.overprint(self.0.stroking_overprint.unwrap_or(false));
            ext_st.overprint_fill(self.0.non_stroking_overprint.unwrap_or(false));
        }

        if let Some(overprint_mode) = self.0.overprint_mode {
            // /OPM 1 combined with overprinting over an ICCBased CMYK colour
            // space is forbidden by PDF/A-2/-3/-4 and PDF/X-4/-4p/-6/-6p (see
            // `ValidationError::OverprintOpmOne`). Under those validators device
            // colours are promoted to ICCBased and `/OPM` only affects CMYK, so
            // register the conformance error whenever the forbidden mode is set
            // with overprinting enabled; the profile table decides if it fires.
            let overprinting = self.0.stroking_overprint == Some(true)
                || self.0.non_stroking_overprint == Some(true);
            if matches!(overprint_mode, OverprintMode::IgnoreZeroChannel) && overprinting {
                sc.register_validation_error(ValidationError::OverprintOpmOne(sc.location));
            }
