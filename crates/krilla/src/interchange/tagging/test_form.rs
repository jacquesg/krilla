...
//! before the moegoe end-to-end tests run.
...
mod tests {
    ...
    /// Pin the PDF serialisation when the Form is constructed via
    /// `set_id` (mimicking moegoe's `tag_kind.set_id(...)` call
    /// AFTER `apply_attribute_overrides` has populated /Role and
    /// /Checked). Reproduces the exact mutation order moegoe uses
    /// in production.
    #[test]
    fn form_serialises_when_id_added_after_role_and_checked() {
        ...
        // After role + checked, moegoe sets the HTML id on the
