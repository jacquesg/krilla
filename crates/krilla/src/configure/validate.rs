...
use xmp_writer::{CustomNamespace, Namespace, RdfCollectionType, XmpWriter};
...
pub enum ValidationError {
    ...
    /// The document contains a JavaScript action — a catalog `/OpenAction`
    /// with `/S /JavaScript`, a document-level `/Names /JavaScript` name-tree
    /// entry, a `/S /JavaScript` catalog additional action, or a JavaScript
    /// action reached through an annotation `/A` target or a widget `/AA` slot.
    ///
    /// PDF/A-1/-2/-3 (ISO 19005-1 §6.6.1, -2/-3 §6.5.1) and PDF/X-1a/-3/-4/-4p
    /// (ISO 15930-4/-6 §6.14, -7 §6.18) forbid JavaScript actions. PDF/A-4
    /// (ISO 19005-4 §6.6.2) and PDF/X-6/-6p (ISO 15930-9 §6.14.2) permit them
    /// to be present in the file.
    ContainsJavaScriptAction(Option<Location>),
    /// The document catalog carries an `/AA` additional-actions dictionary.
    ///
    /// Every PDF/A part forbids the catalog `/AA` entry (its document-event
    /// actions are not among the permitted set); every PDF/X revision forbids
    /// it too (ISO 15930-9 §6.14.3).
    ContainsCatalogAdditionalActions(Option<Location>),
    /// A widget annotation or interactive-form field dictionary carries an
    /// `/AA` additional-actions dictionary (a keystroke `/K`, format `/F`,
    /// validate `/V`, calculate `/C`, focus `/Fo` or blur `/Bl` action).
    ///
    /// The "Trigger events" clause of PDF/A-1/-2/-3 (ISO 19005-1 §6.6.2) and
    /// of every PDF/X revision (ISO 15930-9 §6.14.3; the earlier PDF/X parts
    /// forbid all actions) prohibits a widget or field `/AA` entry outright.
    /// PDF/A-4 permits it (its "Trigger events" note marks the entry as not
    /// recommended).
    ContainsWidgetAdditionalActions(Option<Location>),
    /// A named action other than the four standard names (`NextPage`,
    /// `PrevPage`, `FirstPage`, `LastPage`) was emitted, e.g. `/Print`.
    ///
    /// Every PDF/A part forbids non-standard (non-portable) named actions;
    /// PDF/X forbids all actions.
    NonStandardNamedAction(Option<Location>),
...
impl Accessibility {
    fn prohibits(self, error: &ValidationError) -> bool {
        match (self, error) {
            ...
            // dc:title (WTPDF §8.11.1), the catalog /Lang entry (WTPDF §8.4.4)
            // and embedded-file /Desc (WTPDF §8.14.1) are general provisions
            // mandatory at every WTPDF conformance level, exactly as under
            // UA-2 — not accessibility-only extras.
            (
                Self::UA2 | Self::WTPDF,
                ValidationError::NoDocumentLanguage
                | ValidationError::NoDocumentTitle
            ...
            // UA-2's accessibility-specific requirements; WTPDF relaxes these.
            (
                Self::UA2 | Self::WTPDF,
                ValidationError::MissingAltText(_) | ValidationError::MissingAnnotationAltText(_),
            ) => self == Self::UA2,
            // ISO 14289-2 states a heading `/T` title and a document outline
            // (§8.12.2) as recommendations ("should"), not requirements, so
            // neither is a hard error under PDF/UA-2 or WTPDF (unlike PDF/UA-1).
            (
                Self::UA2 | Self::WTPDF,
                ValidationError::MissingHeadingTitle | ValidationError::MissingDocumentOutline,
            ) => false,
        }
    }
    ...
    fn write_xmp(self, xmp: &mut XmpWriter) {
        match self {
            ...
            // WTPDF 1.0 §6.1: a file conforming to the reuse level shall
            // carry a PDF Declaration whose `pdfd:conformsTo` entry names the
            // reuse-level URI. krilla's WTPDF validator targets the reuse
            // conformance level (the accessibility level additionally requires
            // alt text, DisplayDocTitle and the other UA-2 extras).
            Self::WTPDF => {
                let pdfd = || {
                ...
                let mut bag = xmp
                    .element("declarations", pdfd())
                    .array(RdfCollectionType::Bag);
                let mut li = bag.element().obj();
                li.element("conformsTo", pdfd())
                    .value("http://pdfa.org/declarations/wtpdf/#reuse1.0");
            }
        }
    }
