...
- krilla does not write halftones, the `HTP` key, or transfer functions. 🟢
- krilla may write overprint settings (`OP`/`op`/`OPM`) via `Surface::push_overprint`; PDF/X-1a §6.9 does not restrict overprint, prohibiting only `HTP`, `TR` and `TR2` (and restricting `HT`). 🟢
...
- krilla permits annotations outside the print area via `AnnotationInsidePrintArea`; each Link gets `AnnotationFlags::PRINT`. An annotation's `/C` border colour is **not** validated against the output intent, because an annotation is a non-print element (ISO 15930-7 §3.15, §6.1.3: "Non-print elements may make use of any PDF colour space"). 🟢
...
See `crates/krilla/examples/pdfx_validation_samples.rs`, which emits the embedded `x6` sample. krilla writes PDF version 2.0. Same as PDF/X-4 except:
...
- The output profile's ICC version may be v2 or v4 up to v4.3, via `IncompatibleOutputProfileVersion`. 🟢
- No structural limits apply (PDF 2.0 has no architectural-limits annex). 🟢

## PDF/X-6p (ISO 15930-9)

See `crates/krilla/examples/pdf_x6p.rs`. Same as PDF/X-6 with an external output profile, exactly as PDF/X-4p relates to PDF/X-4.
