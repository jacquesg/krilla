...
/// fit-window flavours defined by the same table, letting an
/// embedder request a fit area other than the default explicit
/// point:
///
...
pub enum FitMode {
    ...
    FitVertical,
    /// `/FitR <l> <b> <r> <t>` — fit the supplied rectangle, given in krilla's
    /// coordinate space (top-left origin, like every other krilla API); krilla
    /// maps it to PDF user space (lower-left origin) at serialisation.
    FitRect(Rect),
}
...
impl Hash for FitMode {
    fn hash<H: Hasher>(&self, state: &mut H) {
        ...
        if let FitMode::FitRect(rect) = self {
            let tsp = rect.to_tsp();
...
impl PartialEq for FitMode {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (FitMode::FitRect(a), FitMode::FitRect(b)) => {
                let at = a.to_tsp();
                let bt = b.to_tsp();
...
impl XyzDestination {
    ...
    ///
    /// This is the entry point an embedder uses when it requests a
    /// fit area other than the default explicit point.
    ...
    pub(crate) fn serialize(
        ...
        match self.0.fit_mode {
            ...
            FitMode::FitRect(rect) => {
                ...
                // the maximum y in PDF user space; the /FitR operator
                // expects `[l b r t]` with `b <= t`.
...
mod tests {
    ...
    // -------------------------------------------------------------------
    // Tests for `XyzDestination::with_fit_mode`, the setter that
    // swaps the destination operator emitted by
    ...
    fn fit_mode_horizontal_emits_fith_operator() {
        ...
        // (10, 20) on a 200pt-tall page flips to y = 180 in PDF space.
        assert!(
            contains(&pdf, b"/FitH 180"),
            "FitMode::FitHorizontal must emit the y-flipped offset /FitH 180"
        );
        assert!(
            !contains(&pdf, b"/XYZ"),
    ...
    fn fit_mode_vertical_emits_fitv_operator() {
        ...
        // /FitV reuses the x coordinate, unchanged by the y-flip.
        assert!(
            contains(&pdf, b"/FitV 10"),
            "FitMode::FitVertical must emit the /FitV 10 offset"
        );
    }
    ...
    fn fit_mode_rect_emits_fitr_operator() {
        let pdf = finish_with_destination(FitMode::FitRect(
            Rect::from_xywh(5.0, 10.0, 30.0, 40.0).unwrap(),
        ));
        assert!(
            contains(&pdf, b"/FitR"),
            "FitMode::FitRect must emit /FitR with the supplied rectangle"
        );
        // Rect(5,10,30,40) flips and normalises to /FitR `[l b r t]`.
        assert!(
            contains(&pdf, b"/FitR 5 150 35 190"),
            "FitMode::FitRect must emit the flipped, normalised /FitR 5 150 35 190"
        );
    }
    ...
    fn fit_mode_default_xyz_preserves_legacy_emission() {
        ...
        // The legacy path emits the y-flipped point with zero zoom.
        assert!(
            contains(&pdf, b"/XYZ 10 180 0"),
            "default XyzDestination must emit /XYZ 10 180 0"
        );
    }
}
