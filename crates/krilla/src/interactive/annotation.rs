...
//! a page with an "annotation". krilla currently supports the following
...
//! - [`StampAnnotation`]: rubber-stamp annotations (ISO 32000-2 §12.5.6.12).
//! - [`SoundAnnotation`]: sound annotations (ISO 32000-2 §12.5.6.16).
//! - [`MovieAnnotation`]: movie annotations (ISO 32000-2 §12.5.6.17,
//!   deprecated in PDF 2.0).
//! - [`ScreenAnnotation`]: screen annotations (ISO 32000-2 §12.5.6.18).
//!
...
impl Annotation {
    ...
    pub(crate) fn serialize(
        ...
        // visible border omits Print outside PDF/A so the border shows on screen
        // but not in print. Text, Markup and Widget annotations are visible page
        // artefacts and should always print — except for a `WidgetAnnotation`
        // opted-in via `with_hidden(true)`, which switches to `/F HIDDEN` so the
        // field carries state without a visible appearance (HTML `<input
        // type="hidden">`).
        if let AnnotationType::Link(l) = &self.annotation_type {
        ...
        if !is_widget {
            // The text-bearing subtypes already wrote their own /Contents
            // from the subtype's `contents` field inside `serialize_type`
            // (which ran first); only fall back to the accessibility alt
            // text when the subtype did not, so the annotation dictionary
            // never carries two /Contents keys.
            let subtype_contents = self.annotation_type.contents();
            if subtype_contents.is_none() {
                if let Some(alt_text) = &self.alt {
                    annotation.contents(TextStr(alt_text));
                }
            }

            // PDF/UA and several PDF/A profiles require a /Contents alternate
            // description on every non-widget annotation. It is satisfied by
            // either the subtype's own contents or the alt text, so fire the
            // error only when the effective /Contents is absent or empty.
            let effective_contents = subtype_contents.or(self.alt.as_deref());
            if effective_contents.is_none_or(str::is_empty) {
        ...
        if let AnnotationType::Widget(w) = &self.annotation_type {
            if !matches!(w.field, WidgetField::RadioGroupChild(_)) {
                ...
                // A `/AA /C` calculate action makes this field part of the
                // document's calculation order (`/AcroForm /CO`, ISO 32000-2
                // §12.7.3 Table 224).
                if w.calculate_action.is_some() {
                    sc.register_calculate_field(root_ref);
                }
            }
        }
...
impl AnnotationType {
    /// The annotation rectangle, common to every annotation subtype.
    pub(crate) fn rect(&self) -> Rect {
        match self {
            AnnotationType::Link(l) => l.rect,
    ...
    /// The subtype's own `/Contents` text, if it manages one. The
    /// text-bearing subtypes write `/Contents` from this field inside
    /// their `serialize_type`; [`Annotation::serialize`] consults it so
    /// the outer alt text does not emit a second `/Contents` key, and so
    /// the PDF/UA alt-text check is satisfied by either source. Link and
...
/// §12.5.6.4 Table 175). Viewers display the corresponding pre-defined
...
/// selection).
///
/// `/QuadPoints` is Required for text-markup annotations since PDF 1.3
/// (ISO 32000-2 Table 182); krilla always emits the array. The PDF 1.6
/// qualifier attaches to link-annotation `/QuadPoints` (Table 176), not
/// to the text-markup subtypes here.
...
impl MarkupAnnotation {
    ...
    fn serialize_type(
        ...
        // `/QuadPoints` is Required for text-markup annotations since
        // PDF 1.3 (ISO 32000-2 Table 182), so it is always emitted —
        // unlike link-annotation `/QuadPoints`, which is PDF 1.6+.
...
/// An AcroForm signature field (`/FT /Sig`) per ISO 32000-2 §12.7.5.5.
///
...
/// §12.7.5.5, Table 236 — `SigFieldLock`).
///
...
/// A non-terminal AcroForm radio-button field (ISO 32000-2 §12.7.5.2.4).
///
...
impl RadioGroupField {
    ...
    /// value toggle together), defined in ISO 32000-2 Table 229 (field
    /// flags specific to button fields). Off by default (unset flag bits
    /// are 0).
...
/// text-specific. See ISO 32000-2 §12.7.5.3, Table 231.
...
/// See ISO 32000-2 §12.7.5.2.2, Table 229.
#[derive(Copy, Clone, Debug, Default, Eq, PartialEq)]
...
/// `MULTI_SELECT` is permissible but single-select is the common
/// case; the multi-select API is provided for completeness. The
/// general `required` / `no_export` / `do_not_spell_check` bits
/// (bits 2 / 3 / 23) are shared with the other field types.
/// See ISO 32000-2 §12.7.5.4, Table 233.
#[derive(Copy, Clone, Debug, Default, Eq, PartialEq)]
...
impl WidgetAnnotation {
    ...
    fn serialize_type(
        ...
        // all of these from the parent (ISO 32000-2 §12.7.5.2.4) and
        ...
        match &self.field {
            WidgetField::Text(text) => {
                ...
                job = AppearanceJob {
                    ...
                    on: AppearanceStream {
                        ...
                        bbox_w,
                        bbox_h,
                        content: build_text_appearance_content(&text.value, bbox_h, da_bytes),
                        uses_helvetica: true,
                    },
                    off: None,
            ...
            WidgetField::Button(button) => {
                ...
                // Derive the radio / pushbutton `/Ff` bits from
                // `button.kind` so the button type is never contradicted
                // by the flags: a Radio sets bit 16, a PushButton sets
                // bit 17, a Checkbox sets neither (ISO 32000-2
                // §12.7.5.2.2, Table 229 — "if clear, the field is a
                // check box"). `button.kind` is authoritative (it also
                // selects the appearance stream), so the opposite bit is
                // cleared; this mirrors the radio-group emitter in
                // `page.rs`, which force-ORs the Radio flag.
                let ff = match button.kind {
                    ButtonKind::Radio => button.flags.with_radio(true).with_pushbutton(false),
                    ButtonKind::PushButton => button.flags.with_pushbutton(true).with_radio(false),
                    ButtonKind::Checkbox => button.flags.with_radio(false).with_pushbutton(false),
                };
                annotation.pair(Name(b"Ff"), ff.to_bits() as i32);
                match button.kind {
                    ButtonKind::Checkbox => {
            ...
            WidgetField::RadioGroupChild(child) => {
                ...
                // the parent (ISO 32000-2 §12.7.5.2.4). No /T, no /FT,
            ...
            WidgetField::Choice(choice) => {
                ...
                opt.finish();
                // /I (selected indices) — ISO 32000-2 §12.7.5.4 Table 234:
                // "shall be used when two or more elements in the Opt array
                // have different names but the same export value or when the
                // value of the choice field is an array." So /I is emitted
                // whenever /V resolves to an array (two or more selected
                // values) or two /Opt entries share an export value —
                // independent of the MultiSelect flag. Iterating /Opt in
                // order yields the indices already sorted ascending.
                let selected_v_is_array = choice.values.len() >= 2;
                let opt_export_collision =
                    choice.options.iter().enumerate().any(|(i, (export, _))| {
                        choice.options[i + 1..]
                            .iter()
                            .any(|(other, _)| other == export)
                    });
                if !choice.values.is_empty() && (selected_v_is_array || opt_export_collision) {
                    let mut indices = annotation.insert(Name(b"I")).array();
                    for (index, (export, _)) in choice.options.iter().enumerate() {
                        if choice.values.iter().any(|value| value == export) {
                            indices.item(i32::try_from(index).unwrap_or(i32::MAX));
                        }
                    }
                    indices.finish();
                }
                ...
                job = AppearanceJob {
                    ...
                    on: AppearanceStream {
                        ...
                        bbox_w,
                        bbox_h,
                        content: build_text_appearance_content(display, bbox_h, da_bytes),
                        uses_helvetica: true,
                    },
                    off: None,
                    icons: WidgetIconXObjects::default(),
...
/// Emit a signature-field lock (`SigFieldLock`) dictionary as its own
/// indirect object, named from the widget's `/Lock` entry. ISO 32000-2
/// §12.7.5.5 Table 235 requires `/Lock` to be an indirect reference, so
/// the dictionary is written as a standalone object rather than embedded
/// inline in the widget dictionary.
fn write_sig_field_lock(chunk: &mut Chunk, lock_ref: Ref, lock: &SignatureLock) {
    let mut dict = chunk.indirect(lock_ref).dict();
    dict.pair(Name(b"Type"), Name(b"SigFieldLock"));
    match lock {
        // Callers guard on a non-`None` lock, so this arm is never
        // reached; it keeps the match exhaustive.
        SignatureLock::None => {}
        SignatureLock::All => {
            dict.pair(Name(b"Action"), Name(b"All"));
        }
        SignatureLock::Include { fields } => {
            dict.pair(Name(b"Action"), Name(b"Include"));
            let mut arr = dict.insert(Name(b"Fields")).array();
            for f in fields {
                arr.item(TextStr(f));
            }
            arr.finish();
        ...
        SignatureLock::Exclude { fields } => {
            dict.pair(Name(b"Action"), Name(b"Exclude"));
            let mut arr = dict.insert(Name(b"Fields")).array();
            for f in fields {
                arr.item(TextStr(f));
            }
...
/// Escape a UTF-8 string for a PDF content-stream literal `(...)`,
/// mapping each codepoint onto its WinAnsiEncoding byte code — the
/// encoding of the shared form font (`/Helvetica` with
/// `/WinAnsiEncoding`). The Latin-1 range (accented letters, `£`, …)
/// and the code-page-1252 punctuation (curly quotes, dashes, `€`, …)
/// therefore bake correctly; only codepoints WinAnsi cannot represent
/// (e.g. CJK) fall back to `?`.
///
/// The `?` fallback is lossy and, under PDF/A or PDF/UA — where
/// `/NeedAppearances` is prohibited and the baked appearance is the
/// only rendering — permanent. `/V` still carries the exact UTF-16
/// value, so a viewer that regenerates the appearance recovers the
/// original text.
fn escape_pdf_string_winansi(value: &str) -> Vec<u8> {
    ...
    for ch in value.chars() {
        match ch {
            ...
            _ => out.push(winansi_encode(ch).unwrap_or(b'?')),
        }
    }
    out
}

/// Map a Unicode scalar onto its WinAnsiEncoding byte code, or `None`
/// when WinAnsi cannot represent it. WinAnsiEncoding coincides with
/// ASCII in `0x20..=0x7E` and with Latin-1 in `0xA0..=0xFF`; the
/// `0x80..=0x9F` block carries the code-page-1252 punctuation and
/// letters enumerated below.
fn winansi_encode(ch: char) -> Option<u8> {
    let cp = ch as u32;
    match cp {
...
/// Rewrite a `/DA` (default appearance) operator string so its font
/// resource is the document Helvetica (`/Helv`) that the baked
/// appearance's `/Resources` binds and the AcroForm `/DR` defines —
/// krilla embeds no other form font. The `/DA`'s size and colour
/// operators are preserved verbatim, so a caller's `/Helv 14 Tf 1 0 0
/// rg` bakes at 14 pt red while a `/Courier …` string degrades to
/// Helvetica at the requested size and colour. Falls back to the
/// built-in default when the string names no font.
fn da_appearance_ops(da: &[u8]) -> Vec<u8> {
    // The font resource is the first PDF name in the /DA operator list;
    // swap it for /Helv and keep the trailing "<size> Tf <colour>" ops.
    if let Some(slash) = da.iter().position(|&b| b == b'/') {
        let after_name = &da[slash + 1..];
        let name_end = after_name
            .iter()
            .position(|&b| b.is_ascii_whitespace())
            .unwrap_or(after_name.len());
        let mut out = Vec::with_capacity(da.len() + 5);
        out.extend_from_slice(b"/Helv");
        out.extend_from_slice(&after_name[name_end..]);
        return out;
    }
    b"/Helv 10 Tf 0 g".to_vec()
}

/// Build a single-line variable-text appearance wrapped in the
/// `/Tx BMC … EMC` marked-content region ISO 32000-2 §12.7.4.3
/// prescribes: `/Tx BMC q BT <DA ops> 2 y Td (value) Tj ET Q EMC`. The
/// font-set-and-colour operators come from the field's effective `/DA`
/// (via [`da_appearance_ops`], normalised onto the bound `/Helv`
/// resource) so the baked appearance honours a caller's
/// [`WidgetAnnotation::with_default_appearance`] rather than hardcoding
/// Helvetica-10-black. The `/Tx` marked content lets a processor
/// updating the field value locate and replace the region in place
/// (rather than appending a second copy). `bbox_h` chooses a baseline
/// ~3pt below the vertical centre.
fn build_text_appearance_content(value: &str, bbox_h: f32, da: &[u8]) -> Vec<u8> {
    let baseline_y = ((bbox_h - 10.0) / 2.0).max(2.0);
    let mut out = Vec::with_capacity(value.len() + da.len() + 64);
    out.extend_from_slice(b"/Tx BMC\nq\nBT\n");
    out.extend_from_slice(&da_appearance_ops(da));
    out.extend_from_slice(b"\n2 ");
    out.extend_from_slice(format!("{:.2}", baseline_y).as_bytes());
    out.extend_from_slice(b" Td\n(");
    out.extend_from_slice(&escape_pdf_string_winansi(value));
...
///
/// The achromatic strokes use DeviceGray operators (`G`), which are
/// PDF/A-conformant under any output intent (ISO 19005-2/-3/-4 §6.2.4.3:
/// `DeviceGray` is permitted whenever a PDF/A OutputIntent is in effect),
/// unlike DeviceRGB which requires an RGB intent / DefaultRGB.
...
/// Build a checkbox "off" stream: just the 1pt black border. Uses
/// DeviceGray (`G`) for PDF/A conformance under any output intent (see
/// [`build_checkbox_on_content`]).
...
fn build_radio_on_content(bbox_w: f32, bbox_h: f32) -> Vec<u8> {
    ...
    // DeviceGray (`G`/`g`) — PDF/A-conformant under any output intent
    // (see `build_checkbox_on_content`).
    out.push_str("q\n0 G\n0 g\n0.5 w\n");
...
fn build_radio_off_content(bbox_w: f32, bbox_h: f32) -> Vec<u8> {
    ...
    let r_outer = (bbox_w.min(bbox_h) / 2.0 - 1.0).max(0.5);
    let mut out = String::new();
    // DeviceGray (`G`) — PDF/A-conformant under any output intent (see
    // `build_checkbox_on_content`).
    out.push_str("q\n0 G\n0.5 w\n");
    append_circle(&mut out, cx, cy, r_outer, "S");
    out.push_str("Q\n");
...
fn build_pushbutton_content(caption: &str, bbox_w: f32, bbox_h: f32) -> Vec<u8> {
    ...
    // DeviceGray (`g`/`G`) for the grey fill and border — PDF/A-conformant
    // under any output intent (see `build_checkbox_on_content`); the
    // caption already uses `0 g`.
    ...
    out.extend_from_slice(format!("{:.2}", baseline_y).as_bytes());
    out.extend_from_slice(b" Td\n(");
    out.extend_from_slice(&escape_pdf_string_winansi(caption));
    out.extend_from_slice(b") Tj\nET\nQ\n");
    out
}
...
mod tests {
    ...
    #[test]
    fn widget_annotation_pushbutton_default_flags_still_sets_bit_17() {
        // Regression (ISO 32000-2 §12.7.5.2.2, Table 229): the
        // Pushbutton `/Ff` bit is derived from `ButtonKind`, so a
        // pushbutton with default flags must still serialise as
        // `/Ff 65536` and never as `/Ff 0` (a check box).
        let button = WidgetField::Button(ButtonField {
            checked: false,
            kind: ButtonKind::PushButton,
            caption: "Submit".into(),
        ...
        assert!(
            ...
            "pushbutton bit must be derived from kind even with default flags"
        );
        assert!(
            !contains(&pdf, b"/Ff 0"),
            "a pushbutton must never serialise as /Ff 0 (a check box)"
        );
    }

    #[test]
    fn widget_annotation_radio_default_flags_still_sets_bit_16() {
        // Regression (Table 229): the Radio `/Ff` bit is derived from
        // `ButtonKind`, so a radio with default flags still emits
        // `/Ff 32768`.
        let button = WidgetField::Button(ButtonField {
            checked: true,
            kind: ButtonKind::Radio,
        ...
        assert!(
            ...
            "radio bit must be derived from kind even with default flags"
        );
    }
    ...
    fn widget_annotation_choice_multi_select_emits_v_array() {
        ...
        // /I carries the ascending zero-based indices into /Opt of the
        // selected export values (red=0, green=1, blue=2) — ISO 32000-2
        // §12.7.5.4 Table 234.
        assert!(
            contains(&pdf, b"/I [0 1 2]"),
            "missing /I selected-indices array for multi-select"
        );
    }
