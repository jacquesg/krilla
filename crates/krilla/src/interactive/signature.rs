...
//!    document with [`Document::set_digital_signature`](crate::Document::set_digital_signature).
...
pub enum SignatureSubFilter {
    /// PKCS#7 detached — the legacy Adobe profile and the most
    /// broadly interoperable default across PDF signing tools.
    /// Bytes in `/Contents` are a DER-encoded
...
/// `/Contents` (ISO 32000-2 §12.8.1, Table 255).
...
/// The `/ByteRange` key that precedes the placeholder array
/// `pdf-writer` emits, e.g. `/ByteRange [0 1000000000 1000000000
/// 1000000000]` — the leading `0` plus three rewritable slots each
/// holding [`BYTE_RANGE_PLACEHOLDER_VALUE`].
///
/// The patcher tolerates both the compact and `pretty` spacings by
/// scanning for this unique key and then for three back-to-back
/// occurrences of the placeholder decimal value, skipping any
...
fn write_decimal_padded(slot: &mut [u8], value: usize) -> KrillaResult<()> {
    // Left-pad with `0`s so the slot width stays constant. The slot is
    // `BYTE_RANGE_FIELD_WIDTH` bytes wide; a value needing more digits
    // than that would overflow a signature byte-range field, so refuse
    // rather than panic (unreachable for any real file — the offsets fit
    // well within i32).
    let s = format!("{:0>w$}", value, w = slot.len());
    if s.len() != slot.len() {
        return Err(crate::error::KrillaError::DigitalSignature(format!(
            "byte-range value {value} does not fit in {} digits",
            slot.len()
        )));
    }
...
mod tests {
    ...
    fn write_decimal_padded_round_trip() {
        let mut slot = [b'_'; 10];
        write_decimal_padded(&mut slot, 12345).unwrap();
        assert_eq!(&slot, b"0000012345");
        write_decimal_padded(&mut slot, 0).unwrap();
        assert_eq!(&slot, b"0000000000");
        write_decimal_padded(&mut slot, 9_999_999_999).unwrap();
        assert_eq!(&slot, b"9999999999");
        // A value needing more than the slot width errors, never panics.
        assert!(write_decimal_padded(&mut slot, 10_000_000_000).is_err());
    }

    #[test]
    fn patch_signature_writes_byte_range_and_swaps_delimiters() {
        // End-to-end check of the post-finish patcher: build a buffer
        // shaped like the relevant slice of a signed PDF, run
        // `patch_signature` with an identity signer, and assert the
        // emitted `/ByteRange` is exactly `[0 a b c]` (a = offset of the
        // `<`, b = first byte after `>`, c = tail length) and that the
        // `/Contents` literal string became a hex string. This guards the
        // second-slot value: it must be `b`, not `b - a`.
        let n = 4usize; // placeholder_size_bytes
        let zeros = "0".repeat(n * 2);
        let buf = format!(
            "%PDF-x\n/ByteRange [0 1000000000 1000000000 1000000000]\n/Contents ({zeros})\ntail"
        )
        .into_bytes();
        let sig_bytes = vec![0xDE, 0xAD];
        let signature = DigitalSignature::new(Box::new(move |_to_sign: &[u8]| Ok(sig_bytes)))
            .with_placeholder_size_bytes(n);
        let out = patch_signature(buf, signature).unwrap();

        let a = out.iter().position(|&b| b == b'<').unwrap();
        let close = out.iter().position(|&b| b == b'>').unwrap();
        assert_eq!(out[close], b'>');
        let b = close + 1;
        let c = out.len() - b;

        // Parse the three rewritten slots from `/ByteRange [0 s0 s1 s2]`.
        let arr = find_subsequence(&out, b"[0 ").unwrap() + 3;
        let nums: Vec<usize> = std::str::from_utf8(&out[arr..arr + 32])
            .unwrap()
            .split_whitespace()
            .take(3)
            .map(|s| s.parse().unwrap())
            .collect();
        assert_eq!(nums, vec![a, b, c]);

        // The signature hex sits at the start of the `/Contents` value,
        // zero-padded to the reservation.
        assert_eq!(&out[a + 1..a + 5], b"dead");
    }
