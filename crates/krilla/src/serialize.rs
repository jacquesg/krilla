...
pub struct SerializeSettings {
    ...
    /// document for colour-managed workflows when no PDF/A or PDF/X
    /// validator is active (or to add an additional intent alongside one).
    /// Per ISO 32000-2 §14.11.5, multiple output intents are allowed
    /// provided every PDF/X intent in the array names the same output
    /// condition.
    ///
...
/// catalogue's `/OutputIntents` entry. The number of components in the
/// profile is preserved through the const generic parameter on the
/// underlying [`ICCProfile`].
#[derive(Clone, Debug)]
...
/// auto-generated by the active PDF/A or PDF/X [`Validator`]. krilla emits
...
/// non-empty informational string are present, matching the contract of
/// [`ExternalOutputProfile`].
#[derive(Clone, Debug)]
...
impl CustomOutputIntent {
    ...
    ///
    /// # Arguments
    /// - `subtype`: the `/S` entry of the output intent dictionary.
    /// - `profile`: ICC destination output profile, embedded as a stream.
    /// - `output_condition_identifier`: the `/OutputConditionIdentifier`
    ///   entry — a well-known identifier for the output condition (e.g.
    ///   `"sRGB"`, `"CGATS TR 001"`, or `"Custom"`).
    /// - `info`: the `/Info` entry — a human-readable informational string.
    ///
    /// # Errors
    ///
    ...
    pub(crate) fn pdf_writer_subtype(&self) -> OutputIntentSubtype<'_> {
        match &self.subtype {
...
impl SerializeContext {
    fn serialize_destination_profiles(&mut self) {
        ...
        let has_custom_intents = !self.serialize_settings.output_intents.is_empty();
        ...
        if subtypes.is_empty() && !has_custom_intents && fallback_cmyk.is_none() {
            return;
        }
        ...
        // Caller-supplied output intents are emitted after any
        // validator-generated entries in the catalogue's `/OutputIntents`
        // array, preserving the order they were declared in.
        let custom_intents = self.serialize_settings.output_intents.clone();
        for intent in custom_intents {
            let oi_ref = self.new_ref();
            let profile_ref = self.register_cacheable(intent.profile().clone());
