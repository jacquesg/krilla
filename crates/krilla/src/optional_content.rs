...
//! marked-content sequence carries `/OC` naming an entry in the
//! page's `/Resources /Properties` dictionary that resolves to the
//! OCG's indirect dictionary, so consumers suppress hidden layers by
//! resolving that name through the resource tree.
...
impl Layer {
    ...
    /// Set the layer's `/Intent` per ISO 32000-2 §8.11.2.3. The
...
/// §8.11.2.3. Distinguishes layers a viewer should honour when
