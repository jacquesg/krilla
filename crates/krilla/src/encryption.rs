...
//! transparently with the file encryption key. The `/Encrypt` dict and the
//! trailer `/ID` strings stay in clear (§7.6.2); when `encrypt_metadata` is
//! `false` the `/Metadata` stream is also written in clear, with an Identity
//! `/Crypt` filter (§7.6.6).
...
/// silently truncated per ISO 32000-2 §7.6.4.3.3 (Algorithm 2.A) step (b).
#[derive(Clone)]
