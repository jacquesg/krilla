...
impl Document {
    ...
    ///
    /// If the document carries no [`SignatureField`](crate::annotation::SignatureField)
    /// widget, [`finish`](Self::finish) still produces a valid signature by
    /// creating a standalone, document-level invisible `/FT /Sig` field — this
    /// is not a no-op.
    pub fn set_digital_signature(&mut self, signature: DigitalSignature) {
